...
- **Correction bug progression QCM + label leçons (11/04/2026)** : `PortailModule.jsx` — deux correctifs. (1) Label "niveaux" → "leçons" dans les cartes thématiques (`ThematiquesEntryView`) : affiche désormais "X / Y leçon(s)" pour une sémantique plus claire depuis l'introduction des leçons (1 leçon = 1 niveau dans l'architecture actuelle). (2) Compteur QCM dans `NiveauxView` : mise en forme visuelle distincte ("Question **X** / Y") pour bien différencier ce compteur de navigation des questions du vrai indicateur de progression global.

- **Présence enseignant type Teams (11/04/2026)** : les enseignants peuvent définir leur statut (En ligne / En réunion / Pas joignable / Déconnecté) visible par les élèves dans le chat. **DB** : colonnes `statut_presence TEXT DEFAULT 'deconnecte'` et `presence_updated_at TIMESTAMPTZ` sur la table `enseignants`. **Portail enseignant** (`EnseignantApp.jsx`) : composant `PresencePicker` — liste déroulante compacte dans la sidebar (couleurs : vert #30d158 / orange #ff9f0a / rouge #ff453a / gris #636366), s'ouvre vers le haut, fermeture au clic extérieur ; statut mis à 'en_ligne' au montage et 'deconnecte' au `beforeunload` / logout. `supabaseEnseignant.js` : fonction `updatePresence(enseignantId, statut)` — PATCH REST sur `enseignants`. **Portail élève** (`PortailMessages.jsx`) : pastille colorée sur l'avatar de chaque enseignant + statut texte coloré dans la liste et dans le header du chat ; poll toutes les 30s via `fetchEnseignantsPresence(ids)` (`supabasePortail.js`). `fetchEnseignantsDeLEleve` inclut désormais `statut_presence` dans le SELECT.

- **Tracking des visites élèves + section Activité admin (11/04/2026)** : suivi automatique des connexions élèves sur le portail. **DB** : table `eleve_sessions` (id UUID PK, eleve_id UUID FK, started_at, last_heartbeat, ended_at, duration_seconds) + RLS anon + 3 fonctions SECURITY DEFINER : `start_eleve_session`, `heartbeat_eleve_session`, `end_eleve_session`. **Portail élève** (`PortailApp.jsx`) : session démarrée au montage, heartbeat toutes les 2 min, session terminée au logout ou `beforeunload`. `supabasePortail.js` : fonctions `startSession`, `heartbeatSession`, `endSession`. **Admin Eleves.jsx** : section "Activité sur le portail" dans la fiche élève — pills filtres (7j/30j/3m/1an), 4 stat cards (Visites, Temps total, Durée moyenne, Dernière déconnexion en format relatif "Il y a Xmin/Xh/Xj"), tableau sessions (Date/Durée/Statut Terminée ou Interrompue). Refresh toutes les 5 min (300 000 ms). "Dernière déconnexion" = `ended_at` si propre, sinon `last_heartbeat`. `supabaseAdmin.js` : `fetchEleveActivite(eleveId, from, to)`.

- **Flux cours simplifié — auto-création niveau (11/04/2026)** : `Cours.jsx` — suppression de l'étape "Ajouter un niveau" lors de l'ouverture d'une leçon. `openLecon` vérifie si la leçon a des niveaux existants ; si aucun, crée silencieusement un niveau (même titre que la leçon, score_requis 80, ordre 1) avant d'aller directement à la vue contenus + QCM. Les leçons existantes avec 1 niveau ne sont pas impactées. Breadcrumb "Retour aux niveaux" corrigé en "Retour aux leçons" quand `selLecon` est défini.
